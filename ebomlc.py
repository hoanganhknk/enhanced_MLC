...
def step_ebomlc(main_net, main_opt, hard_loss_f,
    ...
    logit_s, x_s_h = main_net(data_s, return_h=True)
    pseudo_target_s = meta_net(x_s_h.detach(), target_s)
    loss_s_new = soft_loss_f(logit_s, pseudo_target_s)
    if data_c is not None:
        bs1 = target_s.size(0)
        bs2 = target_c.size(0)
        logit_c = main_net(data_c)
        loss_s2 = hard_loss_f(logit_c, target_c)
        loss_s_new = (loss_s * bs1 + loss_s2 * bs2) / (bs1 + bs2)
    grad_g_mainparam_new = list(torch.autograd.grad(loss_s - loss_s_new, main_net.parameters(), create_graph=True))
    grad_g_metaparam_new = list(torch.autograd.grad(loss_s - loss_s_new, meta_net.parameters(), create_graph=True))
